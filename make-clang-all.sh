...
if [ "$#" == "0" ]; then
  ...
  echo
  echo " INSTALL_DIR    Required first parameter same as CMAKE_INSTALL_PREFIX"
  echo " BUILD_DIR      Optional second parameter, '.' if empty"
  exit 1
fi

# First parameter is install dir
INSTALL_DIR=$1
shift

# Optional second parameter is build dir
if [ -z "$1" ]; then
  BUILD_DIR=.
else
  BUILD_DIR=$1
